...
def detect_edges(image, threshold=0.09):
...
def main():
    ...
    smoothed_jacobi = jacobi_method(original_image, max_iter=5, tol=1e-6)
    
    # Apply Gauss-Seidel smoothing
    smoothed_gauss_seidel = gauss_seidel_method(original_image, max_iter=5, tol=1e-6)
    
    # Detect edges on the original image
    edges_original = detect_edges(original_image, threshold=0.09)
    
    # Detect edges on the Jacobi-smoothed image
    edges_jacobi = detect_edges(smoothed_jacobi, threshold=0.09)
    
    # Detect edges on the Gauss-Seidel-smoothed image
    edges_gauss_seidel = detect_edges(smoothed_gauss_seidel, threshold=0.09)
